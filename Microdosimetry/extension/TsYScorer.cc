...
G4bool TsYScorer::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
	...
	if (ParticleName == "e-")
		...
		flagParticle = 2;
	else if (ParticleName == "He6")
		flagParticle = 2;
	else if (ParticleName == "Li7")
		flagParticle = 3;
	else if (ParticleName == "Be7")
		flagParticle = 4;
	else if (ParticleName == "Be9")
		flagParticle = 4;
	else if (ParticleName == "B10")
		flagParticle = 5;
	else if (ParticleName == "B11")
		flagParticle = 5;
	else if (ParticleName == "C11")
		flagParticle = 6;
...
void TsYScorer::OutputResult()
{
	...
	if (IncludeYF) {
		G4cout << "yF = " << yF << " ( std: " << yF_std << ")"
			   << " keV/um" << G4endl;
		fprintf(ySpecfile, "yF = %.4e (std: %.4e) keV/um\n", yF, yF_std);
	}
	if (IncludeYD) {
		G4cout << "yD = " << yD << " ( std: " << yD_std << ")"
			   << " keV/um" << G4endl;
