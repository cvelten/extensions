// Physics Module for TsEmDNAChemistryExtendedCV
//
// ********************************************************************
// *                                                                  *
...
#include "TsEmDNAChemistryExtendedCV.hh"
...
G4_DECLARE_PHYSCONSTR_FACTORY(TsEmDNAChemistryExtendedCV);

TsEmDNAChemistryExtendedCV::TsEmDNAChemistryExtendedCV()
	: G4VUserChemistryList()
{
	G4DNAChemistryManager::Instance()->SetChemistryList(this);
}

TsEmDNAChemistryExtendedCV::TsEmDNAChemistryExtendedCV(TsParameterManager* pM)
...
void TsEmDNAChemistryExtendedCV::DefineParameters()
{
	...
	if (numberOfReactions > 0) {
		for (int i = 0; i < numberOfReactions; i++) {
			...
			if (!MoleculeExists(reactorA))
				Quit(parName, "\n--- Molecule[A]: " + reactorA + " was not found in database");

			if (!MoleculeExists(reactorB))
				Quit(parName, "\n--- Molecule[B]: " + reactorB + " was not found in database");
			...
			for (int j = 0; j < nbOfProduct; j++) {
				...
				if (product[j] == "noproduct") // Note on comparison of strings: See comment on ConstructReactionTable()
					...
					if (!MoleculeExists(product[j]))
						Quit(parName, "\n--- Molecule[PRODUCT]: " + product[j] + " was not found in database");
					else {
...
G4bool TsEmDNAChemistryExtendedCV::MoleculeExists(G4String name)
{
...
void TsEmDNAChemistryExtendedCV::ConstructMolecule()
{
...
void TsEmDNAChemistryExtendedCV::ConstructDissociationChannels()
{
...
void TsEmDNAChemistryExtendedCV::ConstructReactionTable(G4DNAMolecularReactionTable*
															theReactionTable)
{
	//-----------------------------------
...
void TsEmDNAChemistryExtendedCV::ConstructProcess()
{
		...
		if (moleculeDef != G4H2O::Definition() && moleculeDef != TsScavengerProduct::Definition()) {
			...
			for (size_t u = 0; u < fScavengedMolecules.size(); u++) {
				if (moleculeDef->GetName() == fScavengedMolecules[u]) {
					...
					std::cout << "-- Set scavenging capacity for molecule " << fScavengedMolecules[u] << " of " << fScavengedCapacities[u] * s << " /s " << std::endl;
				}
			}
			for (size_t u = 0; u < fRemoveInMaterialTheseMolecules.size(); u++) {
				if (moleculeDef->GetName() == G4MoleculeTable::Instance()->GetConfiguration(fRemoveInMaterialTheseMolecules[u])->GetDefinition()->GetName())
				{
					TsDNARemoveInMaterial* removeProcess = new TsDNARemoveInMaterial();
					G4MolecularConfiguration* mC = G4MoleculeTable::Instance()->GetConfiguration(fRemoveInMaterialTheseMolecules[u]);
...
void TsEmDNAChemistryExtendedCV::ConstructTimeStepModel(G4DNAMolecularReactionTable*
															reactionTable)
{
...
G4bool TsEmDNAChemistryExtendedCV::IsWaterMolecularConfigurationActivated()
{
	return fSetWaterConfiguration;
}

G4String TsEmDNAChemistryExtendedCV::GetFullParmName(G4String suffix)
{
	return "Ch/" + fName + "/" + suffix;
}

void TsEmDNAChemistryExtendedCV::Quit(G4String parName, G4String message)
{
