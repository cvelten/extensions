// Physics Module for TsEmDNAChemistryCV
//
// ********************************************************************
// *                                                                  *
...
G4_DECLARE_PHYSCONSTR_FACTORY(TsEmDNAChemistryCV);

TsEmDNAChemistryCV::TsEmDNAChemistryCV()
	: G4VUserChemistryList()
{
	G4DNAChemistryManager::Instance()->SetChemistryList(this);
}

TsEmDNAChemistryCV::TsEmDNAChemistryCV(TsParameterManager* pM)
...
void TsEmDNAChemistryCV::DefineParameters()
{
...
G4bool TsEmDNAChemistryCV::MoleculeExists(G4String name)
{
...
void TsEmDNAChemistryCV::ConstructMolecule()
{
...
void TsEmDNAChemistryCV::ConstructDissociationChannels()
{
...
void TsEmDNAChemistryCV::ConstructReactionTable(G4DNAMolecularReactionTable*
													theReactionTable)
{
	G4MolecularConfiguration* OH = G4MoleculeTable::Instance()->GetConfiguration("OH");
	G4MolecularConfiguration* OHm = G4MoleculeTable::Instance()->GetConfiguration("OHm");
...
void TsEmDNAChemistryCV::ConstructProcess()
{
			...
			for (size_t u = 0; u < fScavengedMolecules.size(); u++) {
				if (moleculeDef->GetName() == fScavengedMolecules[u]) {
					...
					if (!fScavengerHasProducts[u])
						scavengerProc->SetReaction(mC, fScavengedCapacities[u]);
					else
						scavengerProc->SetReaction(mC, pr, fScavengedCapacities[u]);
					moleculeDef->GetProcessManager()->AddDiscreteProcess(scavengerProc);
...
void TsEmDNAChemistryCV::ConstructTimeStepModel(G4DNAMolecularReactionTable* reactionTable)
{
...
G4bool TsEmDNAChemistryCV::IsWaterMolecularConfigurationActivated()
{
	return fSetWaterConfiguration;
}

G4String TsEmDNAChemistryCV::GetFullParmName(G4String suffix)
{
	return "Ch/" + fName + "/" + suffix;
}

void TsEmDNAChemistryCV::Quit(G4String parName, G4String message)
{
